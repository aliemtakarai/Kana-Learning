<template>
  <div class="katakana-page">
    <!-- Hero Section -->
    <section class="hero-section">
      <div class="container">
        <div class="hero-content">
          <div class="hero-badge">
            <span class="badge-icon">カ</span>
            {{ t('navigation.katakana') }}
          </div>
          <h1 class="hero-title">{{ t('katakana.title') }}</h1>
          <p class="hero-description">{{ t('katakana.description') }}</p>
          <div class="hero-stats">
            <div class="stat-item">
              <span class="stat-number">46</span>
              <span class="stat-label">{{ t('home.charts.basic') }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-number">20</span>
              <span class="stat-label">{{ t('home.charts.dakuten') }}</span>
            </div>
            <div class="stat-item">
    ...
    <section class="info-section">
      <div class="container">
        <div class="info-grid">
          <div class="info-card">
            <div class="info-icon">ア</div>
            <h3>{{ t('home.charts.basic') }}</h3>
            <p>{{ t('katakana.basicInfo') }}</p>
          </div>
          <div class="info-card">
            <div class="info-icon">ガ</div>
            <h3>{{ t('home.charts.dakuten') }}</h3>
            <p>{{ t('katakana.dakutenInfo') }}</p>
          </div>
          <div class="info-card">
    ...
    <!-- Katakana Charts Section -->
    <section class="charts-section">
      <div class="container">
        <!-- Category Selector -->
        <div class="category-selector">
          <button 
            :class="['category-btn', { active: activeCategory === 'basic' }]"
            @click="activeCategory = 'basic'"
          >
            <span class="btn-number">01</span>
            <div class="btn-content">
              <span class="btn-title">{{ t('home.charts.basic') }}</span>
              <span class="btn-subtitle">46 {{ t('common.total') }}</span>
            </div>
          </button>
          <button 
            :class="['category-btn', { active: activeCategory === 'dakuten' }]"
            @click="activeCategory = 'dakuten'"
          >
            <span class="btn-number">02</span>
            <div class="btn-content">
              <span class="btn-title">{{ t('home.charts.dakuten') }}</span>
              <span class="btn-subtitle">20 {{ t('common.total') }}</span>
            </div>
          </button>
          <button 
            :class="['category-btn', { active: activeCategory === 'handakuten' }]"
        ...
        <div class="kana-grid-container">
          <div class="kana-grid">
            <div 
              ...
              :key="`katakana-${activeCategory}-${index}`"
              class="kana-card"
              @click="showCharacterDetails(char)"
            >
              <div class="card-main">
                <div class="kana-char">{{ char.kana }}</div>
                <div class="romaji-text">{{ char.romaji }}</div>
              </div>
              <div class="card-example">
                {{ char.example[currentLanguage] }}
              </div>
      ...
      <div v-if="selectedCharacter" class="modal-overlay" @click="closeModal">
        <div class="character-modal" @click.stop>
          ...
          <div class="modal-content">
            <div class="modal-char">{{ selectedCharacter.kana }}</div>
            <div class="modal-romaji">{{ selectedCharacter.romaji }}</div>
            <div class="modal-example">
              <h4>{{ t('common.example') }}:</h4>
              <p>{{ selectedCharacter.example[currentLanguage] }}</p>
            </div>
            <button class="play-sound-btn" @click="playSound(selectedCharacter)">
...
const showCharacterDetails = (char: KanaCharacter) => {
...
const playSound = (char: KanaCharacter) => {
